...
class IOHandler:
    ...
    def append_results(self, minibatch, output):
        ...
        self.results['preds'].append(self.preproc_pred(output))

        if self.phase != 'test':
            self.results['labels'].append(minibatch['labels'])

    def preproc_pred(self, pred):
        """
        Format preds to singlelabel classnumber, like in classnumber_classname.csv

        :param pred:
        :return: singlelabel pred
        """
    ...
    def calculate_iteration_metrics(self, minibatch, output, loss, pbar, preproc_time, train_time, idx):
        ...
        """
        output = self.preproc_pred(output)
        if self.phase != 'test':
